...
Mode: Repo-side working protocol update; APP-side local-file sync required

This version supersedes operational usage of v1.3 while preserving its core structure.
...
Within an accepted baseline-hardening phase, Governance does not need to issue per-iteration micro-directives.

Bounded autonomous iteration by Engineering is allowed when:

* phase identity remains explicit
* closed boundaries remain intact
* baseline identity is not being silently replaced
...
# X. Human-Readable Evidence Rule

For substantive runtime modifications, Engineering should provide at least one minimal human-readable validation artifact.

Human-readable evidence is a control surface for complex and emergent-system development.
...
Engineering should minimize artifact size by default.

Heavier artifacts should be produced only when the human asks for expansion or the lighter artifact is insufficient.

---

# XI. Context Minimization Rule
...
# XIII. Baseline-Hardening Escalation Rule

Inside an accepted baseline-hardening phase, Engineering may continue bounded iterations without new Governance prompts until a real gate is reached.

Engineering must escalate back to Governance when:

* seam choice is materially challenged
* baseline identity may change
* work widens into deeper structural redesign
* algorithm rewrite is being considered
* downstream ownership is being redefined
* higher-level personality or doctrine semantics risk recoupling into lower layers

---

# XIV. Phase Escalation Trigger
...
End of Protocol
Sandbox Cross-Thread Collaboration Protocol v1.4
