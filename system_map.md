...
- viewer-local 3D direction readout now centers on `movement` and `posture`; `movement` is the short-window smoothed travel-direction mode, `posture` is a bounded viewer-only maneuver-posture synthesis, and low-speed playback still uses transform-only smoothing rather than rebuilding a synthetic smoothed frame
...
- a debug-only objective-area direction-read investigation record set now exists for `neutral_transit_v1`; in the `objective_reached_tick +/- 5` window, effective flips far more than the old travel-smoothed mode, the widest centered candidate source dominates, and smoothing is not the primary source of the residual
...
- current Panda3D viewer/export toolchain now includes camera-take recording in `viz3d_panda/app.py`, camera snapshot/apply support in `viz3d_panda/camera_controller.py`, and dedicated offscreen video export in `viz3d_panda/export_video.py`, with default export outputs under `analysis/exports/videos/`
