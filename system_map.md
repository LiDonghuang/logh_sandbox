...
- the active `eng/two-layer-spacing-first-bounded-impl` PR branch now adds a first bounded two-layer spacing implementation candidate inside `test_run/`; current branch-only read is that battle can now see the restore line through direct `v4a` test-only reference settings, pre-contact formation stability now matches scope, and the candidate instead surfaces the old hostile-penetration problem during contact rather than supporting a silent baseline upgrade
