...
- a bounded late-terminal first cut now exists locally for the candidate-active `neutral_transit_v1` path: after first post-step in-window confirmation, the expected-position reference frame is frozen and reused on later ticks; current validation read is structurally correct but still not closed
- the current engineering read on that first cut is that the remaining late residual may now be dominated by a frozen-frame / target-center mismatch; the next bounded root-cause question likely needs to separate terminal orientation freeze from terminal centroid freeze
