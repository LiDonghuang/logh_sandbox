...
- the active `eng/two-layer-spacing-first-bounded-impl` PR branch now adds a first bounded two-layer spacing implementation candidate inside `test_run/`; current branch-only read is now split more clearly by the PR #6 follow-up package: neutral-transit shows a semantically real but currently too-rigid formation substrate candidate, while battle contact surfaces the old hostile-penetration problem as a secondary future line rather than supporting a silent baseline upgrade
