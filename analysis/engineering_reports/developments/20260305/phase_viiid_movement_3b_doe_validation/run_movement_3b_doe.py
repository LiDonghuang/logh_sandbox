...
def main():
    ...
    # Runtime constants from baseline settings.
    ...
    for fr in FR_LEVELS:
        for mb in MB_LEVELS:
            ...
            for opp_idx, opp_name in enumerate(first_six_names, start=1):
                ...
                for model in MODELS:
                    run_counter += 1
                    run_id = f"doe_3b_FR{fr}_MB{mb}_PD5_vs_{opp_name}_{model}"
                    ...
                    ar_p90_a = quantile_or_nan(bridge_telemetry.get("AR", {}).get("A", []), 0.90)
                    wedge_p50_a = quantile_or_nan(bridge_telemetry.get("wedge_ratio", {}).get("A", []), 0.50)
                    split_p90_a = quantile_or_nan(bridge_telemetry.get("split_separation", {}).get("A", []), 0.90)
                    env_p90_a = quantile_or_nan(bridge_telemetry.get("angle_coverage", {}).get("A", []), 0.90)
                    ...
                    run_rows.append(
                        {
                            ...
                            "angle_coverage_p90_A": env_p90_a,
                            "determinism_digest": state_digest(final_state),
                        }
                    )
                    if run_counter % 18 == 0:
                        print(f"[progress] runs {run_counter}/162")

    run_table_path = out_dir / "movement_3b_doe_run_table.csv"
    run_fields = list(run_rows[0].keys()) if run_rows else []
    write_csv(run_table_path, run_rows, run_fields)

    # Pairwise delta table by (cell, opponent): v3a-v1, v3b-v1, v3b-v3a
        ...
        delta_rows.append(
            {
                ...
                "delta_end_tick_v3a_minus_v1": d(r3a, r1, "end_tick"),
                "delta_end_tick_v3b_minus_v1": d(r3b, r1, "end_tick"),
                "delta_end_tick_v3b_minus_v3a": d(r3b, r3a, "end_tick"),
                "delta_win_A_v3a_minus_v1": (1 if r3a["winner"] == "A" else 0) - (1 if r1["winner"] == "A" else 0),
                "delta_win_A_v3b_minus_v1": (1 if r3b["winner"] == "A" else 0) - (1 if r1["winner"] == "A" else 0),
                "delta_win_A_v3b_minus_v3a": (1 if r3b["winner"] == "A" else 0) - (1 if r3a["winner"] == "A" else 0),
                "delta_ar_p90_A_v3a_minus_v1": d(r3a, r1, "ar_p90_A"),
                "delta_ar_p90_A_v3b_minus_v1": d(r3b, r1, "ar_p90_A"),
                "delta_ar_p90_A_v3b_minus_v3a": d(r3b, r3a, "ar_p90_A"),
                "delta_wedge_p50_A_v3a_minus_v1": d(r3a, r1, "wedge_p50_A"),
                "delta_wedge_p50_A_v3b_minus_v1": d(r3b, r1, "wedge_p50_A"),
                "delta_wedge_p50_A_v3b_minus_v3a": d(r3b, r3a, "wedge_p50_A"),
                "delta_cut_tick_T_v3a_minus_v1": d(r3a, r1, "tactical_cut_tick_T"),
                "delta_cut_tick_T_v3b_minus_v1": d(r3b, r1, "tactical_cut_tick_T"),
                "delta_cut_tick_T_v3b_minus_v3a": d(r3b, r3a, "tactical_cut_tick_T"),
            }
        )
    delta_table_path = out_dir / "movement_3b_doe_delta_table.csv"
    delta_fields = list(delta_rows[0].keys()) if delta_rows else []
    write_csv(delta_table_path, delta_rows, delta_fields)

    # Cell summary per model.
    cell_rows = []
        ...
        cell_rows.append(
            {
                ...
                "pocket_tick_T_mean": mean_or_nan([r["tactical_pocket_tick_T"] for r in rows]),
    ...
    cell_summary_path = out_dir / "movement_3b_doe_cell_summary.csv"
    cell_fields = list(cell_rows[0].keys()) if cell_rows else []
    write_csv(cell_summary_path, cell_rows, cell_fields)

    # Determinism check: FR8_MB8 vs first opponent, two repeats per model.
    det_rows = []
    ...
    det_path = out_dir / "movement_3b_doe_determinism_check.md"
    det_lines = [
        "# Movement 3B DOE Determinism Check",
        "",
    ...
    # Brief report.
    report_path = out_dir / "movement_3b_doe_comparison_report.md"
    def model_mean(metric):
        out = {}
        for model in MODELS:
            vals = [r[metric] for r in run_rows if r["movement_model"] == model]
            out[model] = mean_or_nan(vals)
        return out

    # winner needs special treatment
    mean_win = {
        model: (sum(1 for r in run_rows if r["movement_model"] == model and r["winner"] == "A") / max(1, sum(1 for r in run_rows if r["movement_model"] == model)))
        for model in MODELS
    }
    mean_ar = model_mean("ar_p90_A")
    mean_wedge = model_mean("wedge_p50_A")
    mean_cut = model_mean("tactical_cut_tick_T")
    mean_contact = model_mean("first_contact_tick")
    ...
    report_lines = [
        ...
        "| metric | v1 | v3a | v3b |",
        "| --- | ---: | ---: | ---: |",
        f"| win_rate_A | {mean_win['v1']:.3f} | {mean_win['v3a']:.3f} | {mean_win['v3b']:.3f} |",
        f"| first_contact_tick_mean | {mean_contact['v1']:.2f} | {mean_contact['v3a']:.2f} | {mean_contact['v3b']:.2f} |",
        f"| cut_tick_T_mean | {mean_cut['v1']:.2f} | {mean_cut['v3a']:.2f} | {mean_cut['v3b']:.2f} |",
        f"| AR_p90_A_mean | {mean_ar['v1']:.3f} | {mean_ar['v3a']:.3f} | {mean_ar['v3b']:.3f} |",
        f"| Wedge_p50_A_mean | {mean_wedge['v1']:.3f} | {mean_wedge['v3a']:.3f} | {mean_wedge['v3b']:.3f} |",
        "",
        "## Notes",
    ...
    print("[done] movement 3B DOE complete")
    print(f"[out] {out_dir}")
