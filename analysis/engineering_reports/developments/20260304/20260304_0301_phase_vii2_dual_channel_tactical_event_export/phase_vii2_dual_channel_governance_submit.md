...
## Notable Mismatches
- Side-level count where `cut_tick_geom` exists but `cut_tick_tactical` never triggers: 1
- Side-level count where `pocket_tick_geom` exists but `pocket_tick_tactical` never triggers: 0
